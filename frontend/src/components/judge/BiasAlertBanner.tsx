"use client";

import React from "react";

export const BiasAlertBanner = ({ flags }: { flags: string[] }) => {
  if (!flags || flags.length === 0) return null;

  return (
    <div className="bg-red-50 border-l-4 border-red-500 p-4 mb-6 rounded-r-xl shadow-sm">
      <div className="flex items-start">
        <svg className="w-6 h-6 text-red-500 mr-3" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth="2" d="M12 9v2m0 4h.01m-6.938 4h13.856c1.54 0 2.502-1.667 1.732-3L13.732 4c-.77-1.333-2.694-1.333-3.464 0L3.34 16c-.77 1.333.192 3 1.732 3z"></path></svg>
        <div>
          <h3 className="font-bold text-red-800 text-sm mb-1">Potential Bias Detected in Feedback</h3>
          <p className="text-xs text-red-700 mb-2">
            Our AI analysis engine has flagged your feedback for potential unconscious bias. Please review your terminology before finalizing.
          </p>
          <ul className="list-disc pl-4 text-xs text-red-600 font-semibold space-y-1">
            {flags.map((flag, idx) => <li key={idx}>{flag}</li>)}
          </ul>
        </div>
      </div>
    </div>
  );
};
