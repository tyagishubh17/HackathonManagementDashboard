      ...
      <div className="lg:col-span-7 space-y-6">
        ...
        <div className="bg-white rounded-2xl border p-6 shadow-sm space-y-4">
          ...
          <div className="border rounded-xl overflow-hidden shadow-inner bg-gray-100">
            <div className="bg-gray-50 border-b px-4 py-2 flex items-center justify-between text-xs font-mono text-gray-500">
              <span>📄 Document Source: {projectData.pptFile?.fileName || "None provided"}</span>
              {projectData.pptFile?.viewUrl && (
                <a href={projectData.pptFile.viewUrl} target="_blank" rel="noreferrer" className="text-indigo-600 hover:underline font-bold">
                  Open in New Tab
                </a>
              )}
            </div>
            {projectData.pptFile?.viewUrl ? (
              <iframe
                src={projectData.pptFile.viewUrl}
                className="w-full h-[580px]"
                title="Submission Layout Viewer"
              />
            ) : (
              <div className="w-full h-[580px] flex items-center justify-center bg-gray-50 text-gray-500 font-bold">
                No PPT file uploaded.
              </div>
            )}
          </div>
        </div>
        ...
        <div className="bg-white p-6 rounded-2xl border shadow-sm space-y-3">
          ...
          {projectData.videoLink && (
            <p className="text-sm"><strong>Video Link:</strong> <a href={projectData.videoLink} target="_blank" rel="noreferrer" className="text-indigo-600 hover:underline font-bold">{projectData.videoLink}</a></p>
          )}
          <p className="text-sm text-gray-600 bg-gray-50 p-4 rounded-xl font-mono text-xs whitespace-pre-wrap border">
            {projectData.summary || projectData.description}
          </p>
        </div>
      </div>
