...
import { Calendar, Target, ShieldCheck, FileText, Clock, Megaphone, Upload, CheckCircle, AlertTriangle, Loader } from "lucide-react";
...
export default function HackathonDetails() {
    ...
    async function loadProjectContext() {
      if (myReg && myReg.status === "confirmed") {
        try {
          ...
          if (matchingTeam && matchingTeam.project) {
            setProject(matchingTeam.project);
            setTitle(matchingTeam.project.title || "");
            setDescription(matchingTeam.project.description || "");
  ...
  const handleFileUpload = async (e: React.FormEvent) => {
    ...
    try {
      ...
      alert("PDF submittal recorded successfully!");
    } catch (err: any) {
  ...
  const handleSubmitProject = async () => {
    if (!project || !project.submissionFiles || project.submissionFiles.length === 0) {
      setFormError("Please upload a specification PDF prior to submission validation checkouts.");
      return;
    }
    try {
      setFormError("");
          ...
          {myReg && myReg.status === "confirmed" && (
            <div className="bg-white rounded-2xl border p-8 shadow-sm space-y-6">
              <div className="border-b pb-4">
                <h2 className="text-2xl font-black text-gray-900">Project Workspace & PDF Submission Panel</h2>
                <p className="text-sm text-gray-500">Manage descriptions and attach layout documentation files here.</p>
              </div>
              ...
              <div>
                <label className="block text-xs font-bold text-gray-700 mb-1">Overview Log Summary</label>
                <textarea value={description} onChange={(e) => setDescription(e.target.value)} disabled={project?.status === "submitted"} rows={3} className="w-full border p-3 rounded-xl text-sm" placeholder="Describe architecture parameters..." />
              </div>
              ...
              {project?.status !== "submitted" && (
                <form onSubmit={handleFileUpload} className="space-y-3 pt-2">
                  <div className="border-2 border-dashed border-gray-200 bg-gray-50 rounded-xl p-4 text-center">
                    ...
                    <input type="file" multiple accept=".pdf" onChange={(e) => setSelectedFiles(e.target.files)} className="mx-auto block text-xs" />
                  </div>
                  <button type="submit" disabled={uploading || !selectedFiles} className="w-full bg-indigo-600 hover:bg-indigo-700 text-white font-bold py-2.5 rounded-xl text-sm transition">
                    {uploading ? "Streaming PDF asset..." : "Upload Specification PDF"}
                  </button>
                </form>
              )}
              ...
              {project?.submissionFiles?.length > 0 && (
                <div className="space-y-1.5 bg-gray-50 p-4 rounded-xl border">
                  <p className="text-xs font-bold text-gray-600">Attached Spec Assets:</p>
                  {project.submissionFiles.map((file: any, index: number) => (
                    <div key={index} className="flex items-center justify-between text-xs bg-white p-2 rounded-lg border">
                      <span className="font-semibold text-gray-700 truncate max-w-[200px]">{file.name}</span>
                      <a href={file.url} target="_blank" rel="noreferrer" className="text-indigo-600 font-bold hover:underline">View Link</a>
                    </div>
                  ))}
                </div>
              )}
              ...
              {project?.status !== "submitted" ? (
                <div className="pt-4 border-t flex flex-col sm:flex-row items-center justify-between gap-4">
                  <p className="text-xs text-gray-500 leading-relaxed">Completing submissions closes edits, extracts file text strings, and assigns split reviewer panels.</p>
                ...
                <div className="bg-green-50 border border-green-200 text-green-800 p-4 rounded-xl font-bold text-center text-sm flex items-center justify-center gap-2">
                  <CheckCircle size={18} /> Project submitted successfully! Evaluators are reviewing your specification PDF.
                </div>
              )}
            </div>
          )}
