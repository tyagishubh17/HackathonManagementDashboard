...
exports.getMyTeams = async (req, res) => {
  try {
    ...
    const teamsWithProjects = await Promise.all(teams.map(async (t) => {
      const doc = t.toObject();
      doc.project = await Project.findOne({ teamId: t._id });
      return doc;
    }));

    res.status(200).json({ success: true, data: teamsWithProjects });
  } catch (err) {
    res.status(500).json({ message: err.message });
  }
};
