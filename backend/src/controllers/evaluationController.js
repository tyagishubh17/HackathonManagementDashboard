...
exports.assignReviewersAI = async (req, res) => {
  try {
    ...
    if (hackathon.panelsFinalized) {
      return res.status(400).json({ message: "Panels have already been finalized and cannot be modified." });
    }
    ...
    // 3. Form two distinct 3-member panels (Panel A and Panel B) by balancing expertise
    const panelA = [];
    const panelB = [];
    
    const judgesWithSkill = judges.map(j => ({
      ...j.toObject(),
      primarySkill: (j.judgeDetails && j.judgeDetails.expertise && j.judgeDetails.expertise.length > 0) ? j.judgeDetails.expertise[0].toLowerCase() : "general"
    }));

    // Sort by skill to distribute identical skills evenly across panels
    judgesWithSkill.sort((a, b) => a.primarySkill.localeCompare(b.primarySkill));

    judgesWithSkill.forEach((judge, idx) => {
      if (idx % 2 === 0) {
        panelA.push(judge);
      } else {
        panelB.push(judge);
...
exports.finalizePanels = async (req, res) => {
  try {
    const hackathon = await Hackathon.findById(req.params.id);
    if (!hackathon) return res.status(404).json({ message: "Hackathon not found" });

    hackathon.panelsFinalized = true;
    await hackathon.save();

    res.status(200).json({ success: true, message: "Panels finalized successfully." });
  } catch (err) {
    res.status(500).json({ message: err.message });
  }
};
...
exports.calculateLeaderboard = async (req, res) => {
  try {
    ...
    const projects = await Project.find({ hackathonId: hackathon._id, status: "submitted" }).populate("teamId", "name");
    ...
    for (const project of projects) {
      ...
      evaluations.forEach(ev => {
        if (ev.status === "submitted" || ev.status === "resolved") {
          humanTotal += ev.totalScore;
          humanCount++;
          if (ev.totalScore > maxHuman) maxHuman = ev.totalScore;
          if (ev.totalScore < minHuman) minHuman = ev.totalScore;
        }
        if (ev.aiSuggestedScores && ev.aiSuggestedScores.size > 0) {
           let sum = 0;
           ev.aiSuggestedScores.forEach(val => sum += val);
           aiTotalScore = sum; 
        }
      });
      ...
      const avgHumanScore = humanTotal / humanCount;
      const finalScore = (avgHumanScore * 0.70) + (aiTotalScore * 0.30);

      let isFlagged = false;
      const biasFlags = [];

      // Flag if discrepancy is >= 50
      if (Math.abs(avgHumanScore - aiTotalScore) >= 50) {
        isFlagged = true;
        biasFlags.push(`AI/Human Discrepancy (Human Avg: ${avgHumanScore.toFixed(1)}, AI: ${aiTotalScore})`);
      }
      
      if (humanCount > 1 && (maxHuman - minHuman) >= 50) {
        isFlagged = true;
        biasFlags.push(`High Human Variance (Max: ${maxHuman}, Min: ${minHuman})`);
      }
...
exports.getMyAssignments = async (req, res) => {
  try {
    const evals = await Evaluation.find({ reviewerId: req.user._id })
      .populate({
        path: "projectId",
        select: "title description techStack pptFile summary videoLink status teamId",
  ...
  try {
    ...
    if (user.role === "participant") {
      ...
      if (hackathon?.resultsPublished) {
        ...
        const populated = await Project.populate(results, { path: "_id", select: "title teamId" });
        return res.status(200).json({ success: true, accessLevel: "participant_leaderboard", data: populated });
      }
...
exports.computeResults = async (req, res) => {
  try {
    const hackathonId = req.params.id;
    const projects = await Project.find({ hackathonId, status: "submitted" });
    ...
    for (const project of projects) {
      ...
      for (const ev of evaluations) {
        ...
        if (score < minHumanScore) minHumanScore = score;
        totalHumanScore += score;

        if (ev.aiSuggestedScores) {
          let aiTotal = 0;
          for (const value of ev.aiSuggestedScores.values()) {
            aiTotal += value;
          }
          if (aiTotal > 0) aiScore = aiTotal;
        }
      }

      const avgHumanScore = totalHumanScore / evaluations.length;
      const finalScore = roundToTwo((avgHumanScore * 0.70) + (aiScore * 0.30));

      let isFlagged = false;
      let flagReason = "";

      if ((maxHumanScore - minHumanScore) > 20) {
        isFlagged = true;
        flagReason = "High variance between human judges.";
      } else if (Math.abs(avgHumanScore - aiScore) > 20) {
        isFlagged = true;
        flagReason = "High variance between AI and humans.";
      }

      // We apply the flag to all evaluations of this project so it's easily visible to the organizer
      await Evaluation.updateMany(
        { projectId: project._id },
        { isFlagged, flagReason, normalizedScore: finalScore }
      );
    ...
    res.status(200).json({ success: true, data: computedProjects });
  } catch (err) {
    res.status(500).json({ message: err.message });
  }
};
