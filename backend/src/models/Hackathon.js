...
const hackathonSchema = new mongoose.Schema(
  {
    ...
    panelsFinalized: { type: Boolean, default: false },
  },
  { timestamps: true }
);
...
hackathonSchema.methods.toPublicJSON = function () {
  return {
    ...
    config: {
      maxTeamSize: this.config?.maxTeamSize || 4,
      minTeamSize: this.config?.minTeamSize || 1,
      allowIndividual: this.config?.allowIndividual || false,
      maxParticipants: this.config?.maxParticipants,
    },
    rubric: this.rubric ? this.rubric.map((r) => ({ criteria: r.criteria, description: r.description })) : [],
    problemStatements: this.problemStatements ? this.problemStatements.map((p) => {
    ...
    stats: { participantsCount: this.stats?.participantsCount || 0 },
